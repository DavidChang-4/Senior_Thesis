...
class E_GCL(nn.Module):
    ...
    def coord_model(self, coord, edge_index, coord_diff, edge_feat):
        ...
        # REPLACED: coord += agg*self.coords_weight
        vel_term2 = agg*self.coords_weight
        return vel_term2
...
class E_GCL_vel(E_GCL):
    ...
    def forward(self, h, edge_index, coord, vel, edge_attr=None, node_attr=None):
        ...
        # REPLACED: coord = self.coord_model(coord, edge_index, coord_diff, edge_feat)
        vel_term2 = self.coord_model(coord, edge_index, coord_diff, edge_feat) # Eq (4) second term



        # REPLACED: coord += self.coord_mlp_vel(h) * vel
        vel_term1 = self.coord_mlp_vel(h) * vel # Eq (4) first term
        next_vel = vel_term1 + vel_term2
        coord += next_vel

        h, agg = self.node_model(h, edge_index, edge_feat, node_attr)
        # coord = self.node_coord_model(h, coord)
        # x = self.node_model(x, edge_index, x[col], u, batch)  # GCN
        return h, coord, next_vel, edge_attr
